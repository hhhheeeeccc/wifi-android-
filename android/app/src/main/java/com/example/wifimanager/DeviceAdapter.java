...
public class DeviceAdapter extends BaseAdapter {
    ...
    public DeviceAdapter(Context c, List<Device> l) {
        this.ctx = c;
        this.list = l;
        this.hm = new HotspotManager(c);
        this.repo = new HotspotRepository(c);
    }
    public void update(List<Device> nl) {
        this.list = nl;
        notifyDataSetChanged();
    }
    @Override public int getCount() { return list != null ? list.size() : 0; }
    @Override public Object getItem(int p) { return list.get(p); }
    @Override public long getItemId(int p) { return (long) p; }
    @Override public View getView(int p, View v, ViewGroup pr) {
        ...
        Button b = (Button) v.findViewById(R.id.btnBlock);
        b.setText(d.isBlocked() ? "Unblock" : "Block");
        ...
        if (d.getDataLimit() > 0) {
            ...
            pg.setProgress((int)Math.min(100, (d.getUsedData()*100L)/d.getDataLimit()));
        }
